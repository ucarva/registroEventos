<x-layouts.admin>
    <div class=" mb-4">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item href="{{ route('admin.asistentes.index') }}">Eventos</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Nuevo</flux:breadcrumbs.item>
        </flux:breadcrumbs>

    </div>
    <div class="bg-white px-6 py-8 rounded-lg shadow-lg space-y-4 ">
        <form action="{{ route('admin.eventos.store') }}" method="POST" class="p-5">
            @csrf
            <flux:input name="titulo" type="text" label="Título del evento" value="{{ old('titulo') }}" />

            <flux:input name="descripcion" type="textarea" label="Descripción" value="{{ old('descripcion') }}" />
            <!-- este input garantiza que se envíe "0" si el checkbox no está marcado -->
            <input type="hidden" name="es_gratuito" value="1">

            <!-- este checkbox enviará "1" si está marcado -->
            <flux:checkbox name="es_gratuito" label="¿Es gratuito?" value="0"
               :checked="old('es_gratuito', false)" />



            <flux:input name="fecha_evento" type="date" label="Fecha del evento" value="{{ old('fecha_evento') }}" />

            <flux:input name="hora_evento" type="time" label="Hora del evento" value="{{ old('hora_evento') }}" />
            ...
            <flux:input name="lugar" type="text" label="Lugar del evento" value="{{ old('lugar') }}" />

            <flux:input name="cupo_disponible" type="number" min="1" label="Cupo disponible"
                value="{{ old('cupo_disponible') }}" />
